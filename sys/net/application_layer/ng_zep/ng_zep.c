...
static int _send(ng_netdev_t *netdev, ng_pktsnip_t *pkt)
{
    ...
    new = _zep_hdr_build(dev, hdr_len + payload_len + 2, false);
    ...
    hdr = ng_udp_hdr_build(new, (uint8_t *)(&(dev->src_port)), sizeof(uint16_t),
    ...
    hdr = ng_ipv6_hdr_build(new, NULL, 0, (uint8_t *) & (dev->dst),
    ...
    mhr_offset = _zep_hdr_fill(dev, zep, payload_len + hdr_len + 2);
    ...
    entry = ng_netreg_lookup(NG_NETTYPE_UDP, NG_NETREG_DEMUX_CTX_ALL);
    ...
    ng_pktbuf_hold(new, ng_netreg_num(NG_NETTYPE_UDP, NG_NETREG_DEMUX_CTX_ALL) - 1);
                                    ...
                                    size_t payload_len)
{
    ...
    vtimer_get_localtime(&time);
    epoch_sec = (uint32_t)mktime(&time);
