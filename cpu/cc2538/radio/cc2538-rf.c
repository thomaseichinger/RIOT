        ...
        if (irq_status0 & FIFOP) {
            if (RFCORE_XREG_RXFIFOCNT < 1) {
                ...
                if (len < CC2538_FCS_LEN) {
                    ...
                    ieee802154_frame_read(rx_buf, &frame, len);
                    ...
                    if (frame.fcf.frame_type != IEEE_802154_ACK_FRAME) {
                        ...
                        if (netdev_rcv_data_cb != NULL) {
                            netdev_rcv_data_cb(
                                ...
                                frame.src_addr,
                                sizeof(frame.src_addr),
                                frame.dest_addr,
                                sizeof(frame.dest_addr),
                                (void *)rx_buf,
                                len
                            );
                        }
...
#endif
                    } /* end if (frame.fcf.frame_type != IEEE_802154_ACK_FRAME) */
                }
            }
        }
    }
