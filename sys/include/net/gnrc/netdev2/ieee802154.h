 ...
 *
 * gnrc_netdev2::recv will expect (but not check) the FCS (2 byte),
 * the LQI (1 byte), and the RSSI (1 byte) of a packet being appended
 * to the the received packet data:
 *
 *      -------------------+---------------+-------+-------+
 *          packet data    |      FCS      |  LQI  | RSSI  |
 *      -------------------+---------------+-------+-------+
 *
 * If the device does not provide any of these informations it is
 * best practice to set the fields to zero.
 *
