...
void cmd_init_master(int argc, char **argv)
{
    ...
    if (res == -1) {
        ...
        printf("SPI_%i successfully initialized as master!\n", dev);
        i2c_dev = dev;
    }
}
...
void cmd_init_slave(int argc, char **argv)
{
    ...
    if (res == -1) {
        ...
        printf("SPI_%i successfully initialized as slave with address %i!\n", dev, addr);
        i2c_dev = dev;
    }
}
...
void cmd_write(int argc, char **argv)
{
    ...
    if (res < 0) {
        ...
        printf("SPI_%i: successfully wrote %i bytes to the bus\n", i2c_dev, res);
    }
}
...
void cmd_write_reg(int argc, char **argv)
{
    ...
    if (res < 1) {
        ...
        printf("SPI_%i: successfully wrote %i bytes to register 0x%02x\n", i2c_dev, res, reg);
    }
}
...
void cmd_read(int argc, char **argv)
{
    ...
    if (res < 1) {
        ...
        printf("SPI_%i: successfully read %i bytes:\n  [", i2c_dev, res);
        for (int i = 0; i < res; i++) {
            printf("0x%02x, ", (unsigned int)data[i]);
        }
        puts("])");
...
void cmd_read_reg(int argc, char **argv)
{
    ...
    if (res < 1) {
        ...
        printf("SPI_%i: successfully read %i bytes from reg 0x%02x:\n  [", i2c_dev, res, reg);
        for (int i = 0; i < res; i++) {
            printf("0x%02x, ", (unsigned int)data[i]);
        }
        puts("])");
