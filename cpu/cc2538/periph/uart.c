...
{
    uint16_t mis;
    const int uart_index = (UART_1_DEV == UART0);
    ...
    mis = UART0->MIS & 0x0000FFFF;

    UART0->ICR = 0x0000FFBF;

    if (mis & (RXMIS | RTMIS)) {
        while (!UART0->FRbits.RXFE) {
            if (uart_config[uart_index].rx_cb != NULL) {
                uart_config[uart_index].rx_cb(uart_config[uart_index].arg, UART0->DR);
            }
            else {
...
{
    uint16_t mis;
    const int uart_index = (UART_1_DEV == UART1);

    ISR_ENTER();

    /* Store the current MIS and clear all flags early, except the RTM flag.
     * This will clear itself when we read out the entire FIFO contents */
    mis = UART1->MIS & 0x0000FFFF;

    UART1->ICR = 0x0000FFBF;

    if (mis & (RXMIS | RTMIS)) {
        while (!UART1->FRbits.RXFE) {
            if (uart_config[uart_index].rx_cb != NULL) {
                uart_config[uart_index].rx_cb(uart_config[uart_index].arg, UART1->DR);
            }
            else {
                /* To prevent an Overrun Error, we need to flush the FIFO even if we
                 * don't have an input_handler. Use mis as a data trash can */
