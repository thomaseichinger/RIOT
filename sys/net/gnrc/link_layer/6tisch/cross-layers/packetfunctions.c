...
void packetfunctions_ip128bToMac64b(
   ...
   memcpy(prefix64btoWrite->addr.prefix, &(ip128b->addr.addr_128b[0]), 8);
   mac64btoWrite->type=ADDR_64B;
   memcpy(mac64btoWrite->addr.addr_64b , &(ip128b->addr.addr_128b[8]), 8);
}
void packetfunctions_mac64bToIp128b(
   ...
   memcpy(&(ip128bToWrite->addr.addr_128b[0]), &(prefix64b->addr.prefix[0]), 8);
   memcpy(&(ip128bToWrite->addr.addr_128b[8]), &(mac64b->addr.addr_64b[0]),  8);
}
...
void packetfunctions_mac64bToMac16b(open_addr_t* mac64b, open_addr_t* mac16btoWrite) {
   ...
   mac16btoWrite->addr.addr_16b[0] = mac64b->addr.addr_64b[6];
   mac16btoWrite->addr.addr_16b[1] = mac64b->addr.addr_64b[7];
}
void packetfunctions_mac16bToMac64b(open_addr_t* mac16b, open_addr_t* mac64btoWrite) {
   ...
   mac64btoWrite->type = ADDR_64B;
   mac64btoWrite->addr.addr_64b[0] = 0;
   mac64btoWrite->addr.addr_64b[1] = 0;
   mac64btoWrite->addr.addr_64b[2] = 0;
   mac64btoWrite->addr.addr_64b[3] = 0;
   mac64btoWrite->addr.addr_64b[4] = 0;
   mac64btoWrite->addr.addr_64b[5] = 0;
   mac64btoWrite->addr.addr_64b[6] = mac16b->addr.addr_16b[0];
   mac64btoWrite->addr.addr_64b[7] = mac16b->addr.addr_16b[1];
}
...
bool packetfunctions_isBroadcastMulticast(open_addr_t* address) {
   ...
   if (address->type==ADDR_128B) {
      if (address->addr.addr_128b[0]==0xff) {
         return TRUE;
      } else {
         return FALSE;
      }
   ...
   for (i=0;i<address_length;i++) {
      if (address->addr.addr_128b[i]!=0xFF) {
         return FALSE;
      }
   }
   return TRUE;
...
bool packetfunctions_isAllRoutersMulticast(open_addr_t* address) {
   if (
      ...
      address->addr.addr_128b[0]  == 0xff &&
      address->addr.addr_128b[1]  == 0x02 &&
      address->addr.addr_128b[2]  == 0x00 &&
      address->addr.addr_128b[3]  == 0x00 &&
      address->addr.addr_128b[4]  == 0x00 &&
      address->addr.addr_128b[5]  == 0x00 &&
      address->addr.addr_128b[6]  == 0x00 &&
      address->addr.addr_128b[7]  == 0x00 &&
      address->addr.addr_128b[8]  == 0x00 &&
      address->addr.addr_128b[9]  == 0x00 &&
      address->addr.addr_128b[10] == 0x00 &&
      address->addr.addr_128b[11] == 0x00 &&
      address->addr.addr_128b[12] == 0x00 &&
      address->addr.addr_128b[13] == 0x00 &&
      address->addr.addr_128b[14] == 0x00 &&
      address->addr.addr_128b[15] == 0x1a
   ) {
      return TRUE;
   }
   return FALSE;
...
bool packetfunctions_isAllHostsMulticast(open_addr_t* address) {
   if (
      ...
      address->addr.addr_128b[13] == 0x00 &&
      address->addr.addr_128b[14] == 0x00 &&
      address->addr.addr_128b[15] == 0x01
   ) {
      return TRUE;
   }
   return FALSE;
...
bool packetfunctions_sameAddress(open_addr_t* address_1, open_addr_t* address_2) {
   ...
   if (memcmp((void*)address_1->addr.addr_128b,(void*)address_2->addr.addr_128b,address_length)==0) {
      return TRUE;
   }
   return FALSE;
}
...
void packetfunctions_readAddress(uint8_t* payload, uint8_t type, open_addr_t* writeToAddress, bool littleEndian) {
   ...
   for (i=0;i<address_length;i++) {
      if (littleEndian) {
         writeToAddress->addr.addr_128b[address_length-1-i] = *(payload+i);
      } else {
         writeToAddress->addr.addr_128b[i]   = *(payload+i);
      }
   }
}
...
void packetfunctions_writeAddress(OpenQueueEntry_t* msg, open_addr_t* address, bool littleEndian) {
   ...
   for (i=0;i<address_length;i++) {
      ...
      if (littleEndian) {
         *((uint8_t*)(msg->payload)) = address->addr.addr_128b[i];
      } else {
         *((uint8_t*)(msg->payload)) = address->addr.addr_128b[address_length-1-i];
      }
   }
}
...
void packetfunctions_calculateChecksum(OpenQueueEntry_t* msg, uint8_t* checksum_ptr) {
   ...
   onesComplementSum(temp_checksum,(idmanager_getMyID(ADDR_PREFIX))->addr.prefix,8);
   onesComplementSum(temp_checksum,(idmanager_getMyID(ADDR_64B))->addr.addr_64b,8);

   // destination address
   onesComplementSum(temp_checksum,msg->l3_destinationAdd.addr.addr_128b,16);
