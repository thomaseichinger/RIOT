 ...
 *
 * @file
 * @brief       Netdev adaption for RS485 drivers
 *
...
static void _receive_data(rs485_t *dev)
{
...
static int _send(ng_netdev_t *netdev, ng_pktsnip_t *pkt)
{
    ...
    if (len == 0) {
        DEBUG("[rs485] error: unable to create 802.15.4 header\n");
        ng_pktbuf_release(pkt);
        return -ENOMSG;
    }
    ...
    if ((ng_pkt_len(snip) + len + 2) > AT86RF2XX_MAX_PKT_LENGTH) {
        printf("[rs485] error: packet too large (%u byte) to be send\n",
...
const ng_netdev_driver_t rs485_driver = {
    .send_data = _send,
